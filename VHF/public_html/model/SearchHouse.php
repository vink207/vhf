    ...
    */
    ...
    {        
        if ($this->number != 0) // huizen gevonden
        {
            if ($this->number == 1)
            {
	           $this->msgnumber .= $this->number . ' vakantiehuis gevonden ';
            }
            elseif ($this->number > 1)
            {
	           $this->msgnumber .= $this->number . ' vakantiehuizen gevonden ';
            }
        }
        else
        {
	       $this->msgnumber .=  'Geen vakantiehuizen gevonden ';
        }
        ...
        {        
            ...
            if (isset($this->dept_id))
            {
                ...
                $this->msgnumber .= '<span style="text-transform:uppercase;">'. $dept_naam .' ('.$streek_naam.')</span>';
            }
            else
            {
                $streek_naam = html_entity_decode($streek_naam);
                $streek_naam = utf8_decode($streek_naam);
                $this->msgnumber .= '<span style="text-transform:uppercase;">'. $streek_naam . '</span>';
            }
    ...
    /**
     * getallen bij checkboxen sidebar search
     */
    private function getCountsSidebar()
    {
        /** tabel sidebar_huis / sidebar */
        $select = "SELECT ";
        $sql = "SELECT name FROM sidebar WHERE actief = 1 ORDER BY categorie ASC";
        $stmt = $this->db->query($sql);
        $sizename = $stmt->rowCount();
        
        while ($row = $stmt->fetch(PDO::FETCH_ASSOC))
        {
            extract($row);
            
            $this->sidebar_counts[$name] = 0;
            $arrname[] = $name;
            $select .= $name.',';
        }
        ...
        for ($x = 0; $x < $z; $x++)
        {
            $huis_id = $this->huisID[$x];
            $sql = $select. " FROM sidebar_huis WHERE huis_id = $huis_id";
	           ...
	           $this->content .= <<<EOD
                <div class="result-item">
                    ...
                    <div class="result-info">
                        <strong>$plaats ($streek_naam) - $code</strong><br />
                        (vanaf $prijs,- p/w) <br /><br />
                        $beschrijving_result <br /><br />
...
EOD;
                ...
                {
                    $this->content .= <<<EOD
                            <li class="float-right">huisdieren toegestaan</li>
EOD;
                }
                // internet
                if ($internet == 1)
...
EOD;
                ...
                <li>$slaapkamers slaapkamers</li>
                    ...
                    <div class="result-image">
                        ...
                        <a href="$url_vakantiehuis" class="bekijk">Bekijk</a>
                        <a href="$url_vakantiehuis" class="boeken">Boeken</a>                    
                    </div>
                  </div>
EOD;
            }
    ...
    {         
        ...
        $response = array('content'=>$this->content, 'filters'=>$this->searchfilters, 'msgnumber'=>$this->msgnumber, 'stringhuisid'=>$stringhuisid, 'sidebar_counts'=>$this->sidebar_counts);
